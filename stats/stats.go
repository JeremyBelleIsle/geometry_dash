...
func (Stats Stats) Draw(screen *ebiten.Image, font *text.GoTextFaceSource, level int, levelName string) {
	...
	gameutil.DrawText(levelName, 190, int(Stats.x)+int(Stats.w)+100, Stats.x+(Stats.w/5), Stats.y+70, 0, screen, color.RGBA{160, 80, 255, 255}, font)
	...
	gameutil.DrawText(fmt.Sprintf("jumps: %d", Stats.Jumps), 120, int(Stats.x)+int(Stats.w), Stats.x+80, Stats.y+(Stats.h/4), 0, screen, color.RGBA{255, 255, 255, 255}, font)

	gameutil.DrawText(fmt.Sprintf("time: %ds", Stats.time/60), 120, int(Stats.x)+int(Stats.w), Stats.x+80, Stats.y+(Stats.h/2.4), 0, screen, color.RGBA{255, 255, 255, 255}, font)
